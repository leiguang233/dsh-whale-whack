...
**本插件版（DSH 插件）已停止更新，这是最后一个版本。**

作者已转向**独立桌面版**，功能更完整（常驻鲸鱼娘、多屏支持、纯净桌面模式、
音效自由选择与导入等），且**不需要安装 DSH**。

👉 **后续更新请看桌面版仓库**：
https://github.com/leiguang233/whale-whack-desktop

（桌面版正在整理上传；仓库地址以本行最终链接为准。）

### 已装用户怎么办？
本插件**仍可正常使用**，不会失效。只是不再有新功能。
想体验新特性请转用桌面版；两者互不影响，可同时安装。
...
- 💬 **丰富吐槽文本**：打中 29 / 没打中 32 条台词（顶部气泡，深海主题配色），部分带 AI 语音
- 🎁 **战绩彩蛋**：累计锤中 / 放跑数（localStorage 持久化），1% 概率弹出蓝色艺术字彩蛋
- ⚙️ **可调设置**：启用 / 冒头速度（**每 6 秒 1~36 只**）/ 音效 / 音量（0~66%，66%=满）
  - 同屏最多 6 只（v0.2.1 起固定，不再提供设置项）
